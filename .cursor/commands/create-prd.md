...
**Obsidian format (required)**: The PRD is an Obsidian vault file. Copy frontmatter from the template and update `title` + `aliases` to match the product. Use `[[wikilinks]]` for any cross-references (e.g. `[[TECH_STACK]]`, `[[ARCHITECTURE_GUIDE]]`).
